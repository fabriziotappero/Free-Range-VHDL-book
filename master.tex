% Free range VHDL
% Authors: Bryan Mealy, Fabrizio Tappero
% Date: May, 2011
%
% (C) 2011 B. Mealy, F. Tappero
%
% !TEX root = master.tex
%
\documentclass[11pt,b5paper]{book}
\usepackage{style}

% LINE SPACING
\renewcommand{\baselinestretch}{1.2} % custom spacing
%\renewcommand{\baselinestretch}{2} % double-line spacing

%%%%%% DOCUMENT BEGIN %%%%%%
\begin{document}
	% front cover
	\thispagestyle{empty}
	\begin{textblock*}{160mm}(5mm,5mm)
	%\textblockcolour{red}
	\includegraphics[width=160mm]{pics/cover1.png}  % full-page picture
	\end{textblock*}
	\null\newpage % latex might neglect \newpage because the page is empty, 
		      % to avoid this use \null\newpage

	% blank page
	\thispagestyle{empty}
	\null\newpage

	% front matter %
	\thispagestyle{empty}
	\begin{textblock*}{60mm}(85mm,90mm)
	%\textblockcolour{red}
	\noindent
	{\sffamily\huge\bfseries Free Range VHDL}\\
	\noindent
	{\sffamily Bryan Mealy, Fabrizio Tappero}\\
	{\color{dark-gray}\rule[5pt]{190pt}{4pt}}
	\end{textblock*}
	\null\newpage

	% license page %
	\thispagestyle{empty}
	\include{license}

	% blank page with note
	\thispagestyle{empty}
	\null
	\vspace*{\fill}
	\begin{center}This page intentionally left blank\end{center}
	\vspace*{\fill}
	\newpage

	% blank page
	\thispagestyle{empty}
	\null\newpage

	% table of contents page %
	\setcounter{page}{5}
	\renewcommand\contentsname{Table of Contents} % rename title
	\pagenumbering{roman}
	\tableofcontents
	%\null\newpage

	% Acknowledgments, from page i
	\include{acknowledgments}

	% blank page
	\thispagestyle{empty}
	\null\newpage

	% Preface, from page ii
	\include{introduction}

	% from page 1 begins Chapter 1 %
	\setcounter{page}{1}
	\pagenumbering{arabic}
	\include{chapter1}  % keith done
	\include{chapter2}  % Keith done
	\include{chapter3}  % Keith done
	\include{chapter4}  % Keith done
	\include{chapter5}  % Keith
	\include{chapter6}  % Keith
	\include{chapter7}  % Keith
	\include{chapter8}  % Mad
	\include{chapter9}  % Mad
	\include{chapter10} % Mad
	\include{chapter11} % Mad
	\include{chapter12} % Mad
	\include{appendix}  % Mad
	\include{authors}   % Mad

\end{document}
%%%%%% DOCUMENT END  %%%%%%


