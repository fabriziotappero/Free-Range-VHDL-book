...
\texttt{signed} and \texttt{unsigned} types are, in a way, similar to \texttt{std\_logic\_vector} types and so the question that you might have is:\\
\textit{Why should I need a \texttt{signed/unsigned} type in place of\\ a \texttt{std\_logic\_vector} type?}\\
The answer to this question is in listing \ref{my_sum}, specifically in line 17 and in line 18. The use of \texttt{signed/unsigned} types is desirable any time your ``begs of bits'' (signals, variables or constants) stop being ``begs'' and become numbers of type signed, unsigned or even integers.
...
As final note, we should mention that the inclusion of the non-standard \texttt{std\_logic\_unsigned} library could have given us the possibility of doing \texttt{out1 <= in1 + 1;} in line 17 of listing \ref{my_sum}, making things much simpler. However, once again, the use of non-standard library is highly discouraged.
...
\begin{my_list}
\item The use of \texttt{signed/unsigned} types is desirable any time your ``begs of bits'' (signals, variables or constants) stop being ``begs'' and become numbers of type signed, unsigned or even integers. A typical example is the variable used for a counter for which there is really no reason to use a \texttt{std\_logic\_vector} type for. Refer to line 33 of Listing \ref{ex26_code}.
...
\item Any use of the non-standard Synopsis libraries: \texttt{std\_logic\_signed}, \texttt{std\_logic\_unsigned} and \texttt{std\_logic\_arith} is highly discouraged.
